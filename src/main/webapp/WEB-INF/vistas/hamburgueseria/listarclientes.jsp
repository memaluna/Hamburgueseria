		...
		<tr>
			...
			<td>			
				<c:forEach items="${p.direcciones}" var="v" varStatus="status">
					<span class="badge bg-warning">${v.calle} ${v.numero} - ${v.localidad}</span>
				</c:forEach>
			</td>
			<td>${p.email}</td>
			<td>${p.fechaAlta}</td>
			<td>
				<a href="/hamburgueseria/cliente/${p.id}">
				...
				<a href="/hamburgueseria/cliente/${p.id}/editar">
				...
				<a href="/hamburgueseria/cliente/${p.id}/borrar" id="btn-borrar">
